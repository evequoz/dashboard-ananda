...
export const CalendarPage = () => {
  ...
  const [events, setEvents] = useState([]); // État pour stocker les événements n8n
  const [loading, setLoading] = useState(true);

  const toggleFullscreen = () => setIsFullscreen(!isFullscreen);

  // RÉCUPÉRATION DES DONNÉES N8N
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setLoading(true);
        // L'URL de production que nous avons validée ensemble
        const response = await fetch('https://n8n.ananda-communaute.cloud/webhook/get-calendar');
        const data = await response.json();
        
        // n8n renvoie { events: [...], advice: "..." }
        if (data && data.events) {
  ...
  return (
    <div className={`flex flex-col gap-4 p-4 transition-all duration-500 ${isFullscreen ? 'fixed inset-0 z-[9999] bg-[#05050a] p-6' : 'h-[calc(100vh-100px)]'}`}>
      ...
      <div className="bg-[#0f0f1a] border border-[#c9a84c]/30 p-4 rounded-2xl flex items-center justify-between shadow-2xl">
        <div className="flex items-center gap-4">
          <div className="bg-[#c9a84c]/10 p-2 rounded-lg">
            <Sparkles className={`w-5 h-5 ${loading ? 'animate-spin text-blue-400' : 'text-[#c9a84c]'}`} />
          </div>
          <div>
            <h1 className="text-xl font-serif text-[#c9a84c]">Cockpit de Planification</h1>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest">
              {loading ? 'Synchronisation IA...' : 'Assistant IA Actif'}
            </p>
          </div>
        </div>
      ...
      {/* ZONE DE L'AGENDA */}
      <div className="flex-1 bg-[#0f0f1a] rounded-3xl border border-[#22223a] p-4 shadow-inner relative">
        {loading && (
          <div className="absolute inset-0 z-10 bg-[#0f0f1a]/50 backdrop-blur-sm flex items-center justify-center rounded-3xl">
        ...
        <FullCalendar
          ...
          // ON UTILISE LES DONNÉES DE N8N ICI
          events={events} 
        />
      </div>
