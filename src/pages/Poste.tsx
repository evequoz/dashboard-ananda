...
} from 'lucide-react';import { useState, useEffect, useCallback } from 'react';
import {
  Mail, Inbox, Briefcase, RefreshCw, CheckCircle,
  AlertCircle, Clock, ChevronRight, Send, Plus,
  Eye, EyeOff, Sparkles, X, Paperclip, Trash2,
...
const ReplyModal = ({ email, accountColor, onSend, onClose, sending, sendStatus, suggestion }: ReplyModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#0f0f1a] border border-[#22223a] rounded-2xl p-6 w-[640px] shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${accountColor}20` }}>
              <Send className="w-4 h-4" style={{ color: accountColor }} />
            </div>
            <div>
              <h3 className="text-sm font-bold text-[#e8e4d9]">Répondre</h3>
              <p className="text-xs text-[#5a587a]">→ {email['Expéditeur']?.replace(/<.*>/, '').replace(/"/g, '').trim()}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-[#5a587a] hover:text-[#e8e4d9] transition-all">
            <X className="w-5 h-5" />
        ...
        {suggestion && (
          <div className="mb-4 p-3 bg-[#7b5ea7]/10 border border-[#7b5ea7]/25 rounded-lg">
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-[#9b7ec7]" />
                <span className="text-xs font-semibold text-[#9b7ec7]">Suggestion IA</span>
              </div>
              <button onClick={() => setText(suggestion)}
                className="text-xs text-[#9b7ec7] hover:text-[#c9a84c] transition-all flex items-center gap-1">
                Utiliser <ChevronRight className="w-3 h-3" />
              </button>
            </div>
            <p className="text-xs text-[#a090c0] leading-relaxed line-clamp-3">{suggestion}</p>
          </div>
        )}

        <textarea value={text} onChange={e => setText(e.target.value)}
          className="w-full h-44 bg-[#05050a] border border-[#22223a] rounded-lg p-4 text-sm text-[#e8e4d9] resize-none focus:outline-none focus:border-[#c9a84c]/40 transition-all"
          placeholder="Rédigez votre réponse..." />

        <div className="flex items-center justify-between mt-4">
          <div>
            {sendStatus === 'success' && (
              <span className="text-xs text-[#4caf7d] flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Envoyé</span>
            )}
            {sendStatus === 'error' && (
              <span className="text-xs text-[#d95555] flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Erreur</span>
            )}
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => onSend(text)} disabled={sending || !text.trim()}
              className="flex items-center gap-2 px-6 py-2 rounded-lg text-sm font-bold transition-all hover:scale-105 disabled:opacity-40"
              style={{ background: `linear-gradient(135deg, ${accountColor}, ${accountColor}cc)`, color: '#05050a' }}>
              {sending ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
              {sending ? 'Envoi...' : 'Envoyer'}
            </button>
    ...
    <div className="flex flex-col -m-8" style={{ height: 'calc(100vh - 80px)' }}>
      ...
      {replyMode && selectedEmail && (
        <ReplyModal
          ...
          sending={sending}
          sendStatus={sendStatus}
          suggestion={selectedEmail['suggestion_reponse'] || ''}
        />
      )}
